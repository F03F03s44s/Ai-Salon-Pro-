#!/bin/bash
# Urban Nail Bar - EVERYTHING for macOS Sierra 10.12.6
# Double-click in Finder. First time if needed:
#   chmod +x "EVERYTHING-MACOS-SIERRA-10.12.6.command"
...
echo "  macOS Sierra 10.12.6 - one click"
echo "============================================"
echo ""
...
case "$OS_VER" in
  10.12.*) ;;
  *)
    echo "NOTE: Tuned for Sierra 10.12.6 - you are on $OS_VER."
    echo ""
    ;;
esac
...
if [ -s "$HOME/.nvm/nvm.sh" ]; then
  . "$HOME/.nvm/nvm.sh"
  nvm use 14 >/dev/null 2>&1 || nvm use 14.21.3 >/dev/null 2>&1 || true
fi

NODE_BIN="$(command -v node || true)"
if [ -z "$NODE_BIN" ]; then
  echo "ERROR: Node.js not found."
  echo "On Sierra 10.12.6 install Node 14 LTS:"
...
if [ "$NODE_MAJOR" -ge 17 ] 2>/dev/null; then
  echo "WARNING: Node $NODE_MAJOR is usually too new for Sierra."
...
# 2) OpenSSH (Pinggy TEMP public URL — ~60 min, not unlimited forever)
